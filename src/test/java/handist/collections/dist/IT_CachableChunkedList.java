...
public class IT_CachableChunkedList implements Serializable {
    ...
    static Place getOwner(long index) {
        int p = ((int)index)/base;
        int off = ((int)index)%base;
        if(off%cbase < 4) return new Place(p-1);
        return null;
    }
    ...
    public void setUp() throws Exception {
        ...
        placeGroup.broadcastFlat(()->{
            int n = here().id + 1;
            for(int i=0;i<n;i++) {
                long from = base * n + cbase * i;
                Chunk<Particle> chunk = new Chunk<>(new LongRange(from, from + numData),
        ...
        try {
            placeGroup.broadcastFlat(() -> {
                ca.forEach((long index, Particle p)->{
                    p.force++;
                    DistLog.log("0forEach0:"+index, p, null);
                } );
                ca.forEach(subRange2, (long index, Particle p)->{
                    p.force++;
                    DistLog.log("0forEach1:"+index, p, null);
                } );
                ...
                DistLog.log("OforEach2sum", x.get(), null);
        ...
        try {
            placeGroup.broadcastFlat(() -> {
                ...
                ca.forEach((Particle e)->{
                    DistLog.log("1Shared:" + e.pos, e, null);
                });
                ca.shared.forEachChunk((RangedList<Particle> chunk) ->{
                    assertTrue(subRange2.contains(chunk.getRange()));
                    DistLog.log("1Shared:"+chunk.getRange(), ca.getSharedOwner(chunk), null);
                });
    ...
    public void testForBcast(final CachableChunkedList<Particle> ca) throws Throwable {
        try {
            placeGroup.broadcastFlat(() -> {
                ...
                ca.forEach((long index, Particle p)-> {
                    DistLog.log("2BcastBefore:" + p.pos, p, null);
                    if (getOwner(index).equals(here()) && subRange1.contains(index)) assertEquals(p.force, 10);
                    else assertNotEquals(p.force, 10);
                });
                ...
                ca.forEach((long index, Particle e)->{
                    if(subRange1.contains(index)) assertEquals(e.force, 10);
                    else assertNotEquals(e.force, 10);
                    DistLog.log("2BcastDONE:" + e.pos, e, null);
                });
            });
        } catch (final MultipleException me) {
            me.printStackTrace();
        ...
        try {
            placeGroup.broadcastFlat(() -> {
                ...
                ca.forEach((Particle e) -> {
                    ...
                    DistLog.log("3ReduceBefore:" + e.pos, e, null);
                });
                ...
                ca.forEach((Particle e) -> {
                    ...
                    DistLog.log("4ReduceAfter:" + e.pos, e, null);
                });
            });
        } catch (final MultipleException me) {
            me.printStackTrace();
        ...
        try {
            placeGroup.broadcastFlat(() -> {
                ...
                ca.forEach((Particle e) -> {
                    ...
                    DistLog.log("5allReduceBefore:" + e.pos, e, null);
                });
                ca.allreduce(Collections.singletonList(subRange1), (Particle p) -> p.force, (Particle p, Long v) -> {
                    p.force += v;
                });
                ca.forEach((long index, Particle p) -> {
                    if(subRange1.contains(index)) assertEquals(p.force, 400);
                    else assertEquals(p.force, 100);
                    DistLog.log("6AlleduceAfter:" + p.pos, p, null);
                });
            });
        } catch (final MultipleException me) {
            me.printStackTrace();
