 ...
 * @param <D> type used to identify elements for relocation, may be identical to K
...
class GlbOperation<C extends AbstractDistCollection<T, C>, T, K, D, R> implements Serializable {
	 ...
	 *
	 * @param c  distributed collection
	 * @param op method to call on each local host to perform the computation
	 * @param p  provider of the list of "keys" for the distributed collection
	 * @param f  object that is presented to the programmer inside the GLB program
	 *           in which the result will be stored.
	 */
	GlbOperation(C c, SerializableConsumer<K> op, DistFuture<R> f,
			SerializableSupplier<GlbTask> glbTaskInit) {
		collection = c;
		operation = op;
